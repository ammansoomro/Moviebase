...
function AllMovies() {
  ...
  useEffect(() => {
    const getFeaturedMovies = async () => {
      const url = "https://yts.torrentbay.to/api/v2/list_movies.json?sort_by=year&limit=20&page=1";
      console.log(url);
  ...
  const fetchMovies = async (CurrentPage) => {
    const url = "https://yts.torrentbay.to/api/v2/list_movies.json?sort_by=year&limit=20&page=" + CurrentPage
    console.log(url);
    const response = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`).then((response) => response.json());
    let data = JSON.parse(response.contents);
    console.log(data);
